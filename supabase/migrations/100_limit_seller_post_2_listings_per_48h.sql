-- Limit sellers: max 2 listings posted within 48 hours.
-- Enforced at DB level (prevents bypassing the UI/API).
...
BEGIN
  ...
  IF COALESCE(v_count, 0) >= 2 THEN
    RAISE EXCEPTION 'You can only post 2 listings within 48 hours';
  END IF;
