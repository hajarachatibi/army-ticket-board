-- 1) Set suite = true for all listings by dchase718@gmail.com.
UPDATE public.listings
SET suite = true
WHERE seller_id IN (
  SELECT id FROM public.user_profiles
  WHERE email = 'dchase718@gmail.com'
);

-- 2) Explicit 3-report rule for Suite (Standard/VIP/Suite = 3; Loge = 7). Logic unchanged; comments updated.
...
DECLARE
  ...
  v_scam_count int;
BEGIN
  -- Only Loge gets 7; Standard, VIP, and Suite use 3.
  ...
  -- Other reasons: remove at threshold (3 for Standard/VIP/Suite, 7 for Loge).
  IF COALESCE(v_other_count, 0) >= v_threshold THEN
    v_remove := true;
    v_report_count := v_threshold;
  -- "Not face value": remove at threshold (3 for Standard/VIP/Suite, 7 for Loge).
