-- All Listings: push locked/sold to bottom for clarity.
-- Keep original ordering for active listings (created_at DESC), so when a listing is released it returns to its old position.
