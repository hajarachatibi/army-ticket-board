 ...
 * Used by both the cron route (in-process, no self-fetch) and the process-push API route.
 */
...
export function isPushConfigured(): boolean {
  return FCM_OK || WEB_PUSH_OK;
}

export type ProcessPushResult =
  | { ok: true; push: { sent: number; errors: number }; listingAlerts: { sent: number; errors: number } }
  | { ok: true; push: "skipped"; listingAlerts: "skipped"; reason: string };

export async function runProcessPush(supabase?: SupabaseClient): Promise<ProcessPushResult> {
  ...
  if (notifications?.length) {
    ...
    for (const n of notifications) {
      ...
      if (!tokensMap.has(uid)) {
        const { data: tokens } = await db.from("push_tokens").select("token").eq("user_id", uid);
      ...
      if (!pushEnabled) {
        await db.from("user_notifications").update({ push_sent: true }).eq("id", n.id);
        continue;
      }
      ...
      for (const token of tokens) {
        ...
        if (r.success) {
          ...
          pushErrors++;
          if (r.error === "invalid_token") await db.from("push_tokens").delete().eq("token", token);
        }
      }
      ...
      // Only mark push_sent when we actually delivered to at least one device, or user has no devices (don't retry forever)
      const hasDevices = tokens.length > 0 || subs.length > 0;
      if (anySent || !hasDevices) {
        await db.from("user_notifications").update({ push_sent: true }).eq("id", n.id);
      }
    }
  }
  ...
  let alertErrors = 0;
  if (listings?.length) {
    const { data: alertPrefs } = await db
      .from("listing_alert_preferences")
      .select("user_id, continent, city, listing_type, concert_date")
      .eq("enabled", true);
    const { data: sentRows } = await db.from("listing_alert_sent").select("listing_id, user_id");
    ...
    for (const l of listings) {
      ...
      for (const prefs of alertPrefs ?? []) {
        ...
        const { data: tokens } = await db.from("push_tokens").select("token").eq("user_id", prefs.user_id);
        for (const t of tokens ?? []) {
          ...
          if (r.success) alertSent++;
            ...
            alertErrors++;
            if (r.error === "invalid_token") await db.from("push_tokens").delete().eq("token", t.token);
          }
        }
        const { data: subs } = await db
          .from("push_subscriptions")
        ...
        await db.from("listing_alert_sent").insert({ listing_id: l.id, user_id: prefs.user_id });
        sentSet.add(key);
      }
    }
  }
