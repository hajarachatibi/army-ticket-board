...
import { LISTING_REPORT_REASONS } from "@/components/ListingReportModal";

export default function MerchListingReportModal({
  open,
  onClose,
  listing,
  onReported,
  ...
  open: boolean;
  onClose: () => void;
  listing?: { merchListingId: string; summary: string } | null;
  onReported?: () => void;
}) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/reports/merch-listing", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          merchListingId: listing.merchListingId,
          reason,
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="merch-listing-report-title"
      onClick={handleClose}
    >
      <div
      ...
      >
        <h2 id="merch-listing-report-title" className="font-display text-xl font-bold text-army-purple">
          Report merch listing
        </h2>
